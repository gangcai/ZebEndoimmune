...
#select clusters
celltype.data=read.table("../../cellClusters/cell_cluster_ids.tsv",header=F,sep="\t")
cells=celltype.data[celltype.data[,2] %in% c(0,1,2),1]
raw.data.merged.select= raw.data.merged[,cells]

all.cells=colnames(raw.data.merged.select)
...
zeb <- CreateSeuratObject(raw.data.merged.select, meta.data = metadata, project = "zeb3k", min.cells = 3, min.features = 200)
...
#filter
zeb <- subset(zeb, subset = nFeature_RNA > 200 & nFeature_RNA < 5000 & percent.mt < 10 & nCount_RNA < 50000)
...
pc.num=25
zeb.integrated <- RunPCA(zeb.integrated, npcs = pc.num, verbose = FALSE)
zeb.integrated <- RunUMAP(zeb.integrated, reduction = "pca", dims = 1:pc.num)
zeb.integrated <- RunTSNE(zeb.integrated, dims = 1:pc.num, verbose = FALSE)
zeb.integrated <- FindNeighbors(zeb.integrated, dims = 1:pc.num, verbose = FALSE)
zeb.integrated <- FindClusters(zeb.integrated,resolution = 0.08,  verbose = FALSE)
